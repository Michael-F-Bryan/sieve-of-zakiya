/*
 ...
 generator P3. On Linux to compile do: $ g++ SoZP3za.cpp -o sozp3
...
#define gen 3   // output designator
      ...
      cout << "Initialize variables and arrays. \n";
      double start_time = currtime();  // begin timing total process      

      num = val-1 | 1;                 // if value even number then subtract 1
      k=num/6; modk = k*6; r=0;        // kth residue group, base num value
      if (num == modk+5){r += 1;}      // find last pc position <= num
      ...
      cout << "Perform sieve on prime candidates. \n";
	 ...
	 nonprmpos = (prime*(modk+7))/3 - 1;
      ...
      cout << "Allocated size for primes = " << (unsigned int) max << "\n";
      ...
      cout << "Init    time (P" << gen << ") = " << (soz_start-start_time)/USECS << "\n";
      cout << "Sieve   time (P" << gen << ") = " << (soz_end-soz_start)/USECS << "\n";
      cout << "Primes  time (P" << gen << ") = " << (end_time-soz_end)/USECS << "\n";
      cout << "Elapsed time (P" << gen << ") = " << (end_time-start_time)/USECS << "\n";
      cout << "Last prime (of " << n << ") = " << primes[n] << "\n";
      return (primes[n]);
}
